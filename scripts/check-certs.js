...
function printCertificateDetails(chain, trusted) {
  ...
  console.log('');
  if (trusted) {
    console.log('✅ Node trusts this certificate using the default system trust store.');
  } else {
    console.log('🚫 Node does NOT trust this certificate with the current system trust store.');
  }
  console.log('');
}
...
function printRemediation(hostname) {
  ...
  console.log('');
  console.log('⚠️ For local development only, set ALLOW_INSECURE_TLS=true in .env.local to bypass validation temporarily.');
  console.log('   NEVER use ALLOW_INSECURE_TLS in CI or production.');
  console.log('');
  console.log('🚫 Do NOT disable TLS verification globally (for example by setting Node\'s rejectUnauthorized flag to 0).');
  console.log('');
}
...
async function inspectCertificate(urlString) {
  ...
  if (url.protocol !== 'https:') {
    console.log('ℹ️  Non-HTTPS URL detected. TLS certificates only apply to HTTPS endpoints.');
...
async function main() {
  ...
  console.log('🔍 TLS Certificate Checker');
  console.log('='.repeat(60));
  ...
  try {
    ...
    if (!chain.length) {
      console.log('⚠️ No certificate information was returned by the server.');
      console.log('');
    ...
    if (trusted) {
      console.log('🎉 TLS validation succeeds. No additional action required.');
      console.log('');
      process.exit(0);
    }

    console.log('❌ TLS validation failed using Node\'s default trust store.');
    if (authError) {
    ...
    console.error('❌ Failed to complete TLS inspection.');
